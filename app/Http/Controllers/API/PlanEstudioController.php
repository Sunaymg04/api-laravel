...
class PlanEstudioController extends Controller
{
    ...
    public function modificar(Request $request, string $id)
    {
        ...
        $resultado = DB::transaction(function () use ($request, $planOrigen, $nombrePlan, $programa, $resumenCambios, $estructuraSnapshot) {
            ...
            if ($estructuraSnapshot) {
                $estructuraSnapshot = $this->materializarEstructuraEditada(
                    $request->input('estructura_editada', []),
                    (int) $request->id_prog_form
                );
            }
    ...
    private function estructuraSnapshotDesdeRequest(Request $request): ?array
    {
        ...
        return [
            'datos_base' => [
                ...
                'Curso' => DB::table('curso')->where('id', $request->id_curso)->value('curso')
                    ?? DB::table('curso')->where('id', $request->id_curso)->value('nombre'),
            ],
    ...
    private function materializarEstructuraEditada(array $estructura, int $programaId): array
    {
        ...
        return $this->snapshotDesdeEstructuraMaterializada($estructuraMaterializada, $programaId);
    }
    ...
    private function resolverDisciplina(array $disciplinaData): Disciplina
    {
        ...
        if (is_numeric($id) && !($disciplinaData['is_new'] ?? false)) {
            return Disciplina::findOrFail((int) $id);
        }
    ...
    private function resolverAsignatura(array $asignaturaData): Asignatura
    {
        ...
        if (is_numeric($idExistente)) {
            return Asignatura::findOrFail((int) $idExistente);
        }

        if (is_numeric($id) && !($asignaturaData['is_new'] ?? false)) {
            return Asignatura::findOrFail((int) $id);
        }
    ...
    private function snapshotDesdeEstructuraMaterializada(array $estructura, int $programaId): array
    {
        $request = new Request([
        ...
        return $this->estructuraSnapshotDesdeRequest($request);
    }
    ...
    private function snapshotPlan(PlanEstudio $plan): array
    {
        ...
        return [
            'datos_base' => [
                ...
                'Curso' => $plan->curso?->curso ?? $plan->curso?->nombre,
            ],
            'curriculos_detalle' => $curriculosDetalle,
            'disciplinas_detalle' => $disciplinasDetalle,
            'asignaturas_detalle' => $asignaturasDetalle,
    ...
    private function compararAsignaturas(array $origen, array $nuevo): array
    {
        ...
        foreach ($nuevo as $label => $detalleNuevo) {
            ...
            if (($origen[$label]['anios'] ?? []) !== ($detalleNuevo['anios'] ?? [])) {
                $detalles[] = [
                    'campo' => 'años académicos',
                    'antes' => implode(', ', $origen[$label]['anios'] ?? []),
                    'despues' => implode(', ', $detalleNuevo['anios'] ?? []),
                ];
            }
